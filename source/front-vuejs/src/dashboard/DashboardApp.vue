    ...
    <div id="app" :class="{ hidden: !isAuthenticated }">
      ...
      <main>
        ...
        <section id="container-list">
          <div v-if="myContainers && myContainers.length === 0" class="empty-placeholder">
            No containers yet.
          </div>
          <div v-else>
        ...
        <h2 class="container-title" id="other-container-title" v-if="otherContainers && otherContainers.length > 0">
          Other containers
        </h2>
...
const _loaderCount = computed(() => loaderRef.value ?? 0);

const theme = ref<"light" | "dark">(getCurrentTheme());
const _isDarkTheme = computed(() => theme.value === "dark");
...
function _pauseAlert(id: number) {
...
function _openConsole(container: ContainerSummary) {
...
function _openMetrics(container: ContainerSummary) {
	currentContainerId.value = container.id;
	if (isSmallScreen()) {
		window.open(
...
function _openConsoleFullScreen() {
...
function _openMetricsFullScreen() {
	if (!currentContainerId.value) return;
	window.open(
...
async function _powerOn(container: ContainerSummary) {
	try {
...
async function _powerOff(container: ContainerSummary) {
	try {
...
async function _deleteContainer(container: ContainerSummary) {
	if (!window.confirm(`Delete VM "${container.id} — ${container.name}"?`))
		return;
	try {
...
function _openCreateModal() {
	...
	void (async () => {
		try {
			...
			console.warn(
				"Unable to refresh user data before creating container",
				error,
			);
		}
	})();
...
function _canAffordType(type: ContainerType) {
...
function _typeName(type: ContainerType) {
	return type.container_type_name || type.name || `Type #${type.id}`;
}

function _typeSpecs(type: ContainerType) {
...
function _createActionLabel(type: ContainerType) {
...
async function _createContainer(type: ContainerType) {
...
const notifyHandler = (event: Event) => {
	...
	if (custom.detail?.message) {
		addAlert(
			...
			custom.detail.type ?? "info",
			custom.detail.autoClose ?? true,
		);
	}
};
