...
from internal_config.config_utils import get_config_values

from .models import ResourceQuota, FileTemplate, FileTemplateItem
...
def create_user_quota(sender, instance, created, **kwargs):
    ...
    """
    default_values = get_config_values(
        [
            "default_vcpus",
            "default_mem_mib",
            "default_disk_gib",
    ...
    if created:
        ResourceQuota.objects.create(
            user=instance,
            max_cpu_percent=int(default_values.get("default_vcpus", "1")),
...
def create_default_file_templates(sender, **kwargs):
    ...
    if getattr(sender, "name", None) != "docker_manager":
        return
